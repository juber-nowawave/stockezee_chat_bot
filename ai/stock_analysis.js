...
const isSectorComparisonQuery = (query) => {
  const patterns = /\b(outperform|underperform|sector\s+perform|vs\s+sector|vs\s+index|beat\s+sector|lag\s+sector|relative\s+to\s+sector|relative\s+perform|sector\s+return|sector\s+comparison|how\s+is.*compared|compare.*sector|index\s+return|benchmark|nifty\s+(bank|it|pharma|energy|auto|metal|fmcg|infra|realty|50)|sector\s+outperform|sector\s+underperform)\b/i;
  return patterns.test(query.trim());
};

// ── Sector Performance Analyst ────────────────────────────────────────────────
const runSectorAnalysis = async (model, stockContext, userQuery, symbol, current_date) => {
  const historicalData = stockContext.historical_data ?? [];
  const peers = stockContext.peers ?? [];
  const companyDetails = stockContext.company_details ?? {};
  const currentPrice = stockContext.current_price_data ?? {};

  const sectorAnalysisPrompt = `You are a professional equity research analyst specializing in NSE and BSE listed companies.
Your task is to determine whether ${symbol} is currently outperforming or underperforming its sector.
Today's date: ${current_date}.

STRICT RULES:
1. NEVER compare absolute share prices across companies.
2. NEVER infer sector performance from price levels.
3. NEVER reference the S&P 500 or any foreign index.
4. ALWAYS use percentage returns for comparison.
5. If sector index return data is not present in the data provided, state this clearly and use peer average returns as a proxy — label it explicitly as "Peer Average (sector proxy)".
6. Output ONLY valid HTML wrapped in <div class="response-content">...</div>. No markdown. No text outside the div.

FIVE-STEP FRAMEWORK:

STEP 1 — SECTOR BENCHMARK
From the company data below, identify the sector/industry and map it to the correct Nifty sectoral index:
  Banking/NBFC → Nifty Bank | Nifty Financial Services
  IT/Software → Nifty IT
  Pharma/Healthcare → Nifty Pharma | Nifty Healthcare
  Energy/Oil & Gas → Nifty Energy | Nifty Oil & Gas
  Auto → Nifty Auto
  Metal/Mining → Nifty Metal
  FMCG → Nifty FMCG
  Infra/Real Estate → Nifty Infra | Nifty Realty
  Default → Nifty 50
State which benchmark you are using and WHY.

STEP 2 — RETURN COMPUTATION
Using ONLY the historical price data provided, compute the stock's % returns:
  • 1-week return: (latest close − close 5 sessions ago) / close 5 sessions ago × 100
  • 1-month return: (latest close − oldest close in data) / oldest close × 100
Show your arithmetic transparently (start price → end price → % return).
If sector index returns are NOT in the data, compute peer average returns from the peer data using the same method and label it as "Peer Average (sector proxy)".

STEP 3 — RELATIVE PERFORMANCE
Relative Performance = Stock Return − Benchmark/Peer-Average Return (same period)
Positive → Outperforming | Negative → Underperforming | Near zero (±1%) → Tracking sector.
Provide this for each timeframe you computed.

STEP 4 — CONSISTENCY CHECK
Validate: do the price levels, returns, and peer data tell a consistent story? Flag any contradictions.

STEP 5 — OUTPUT (HTML)
Format your response as:
  <h2>Sector Benchmark</h2>          — which index + why
  <h2>Return Comparison</h2>         — table: stock vs benchmark/peers for each period
  <h2>Relative Performance</h2>      — α (alpha) per period, direction verdict
  <h2>Final Analyst Conclusion</h2>  — 1–2 sentences, confident and balanced
Close with the mandatory SEBI compliance disclaimer verbatim:
...
${historicalData.length > 0 ? JSON.stringify(historicalData, null, 2) : "NOT AVAILABLE"}

[PEER COMPARISON DATA]
${peers.length > 0 ? JSON.stringify(peers, null, 2) : "NOT AVAILABLE"}

USER QUESTION: "${userQuery}" — regarding ${symbol}

Generate the sector performance analysis HTML now:`;

  const response = await Promise.race([
    model.invoke(sectorAnalysisPrompt),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("Sector analysis timeout")), 90000)
    ),
  ]);

  let result = response?.content?.trim();
  if (!result) throw new Error("Empty sector analysis response");

  if (result.includes("</reasoning>")) {
    result = result.split("</reasoning>")[1]?.trim() || result;
  }
  return result;
};

// ── Response Auditor ─────────────────────────────────────────────────────────
const runResponseAuditor = async (model, originalResponse, userQuery, symbol) => {
  const auditPrompt = `You are an expert financial analysis editor and AI response auditor.
Your job is to review and improve responses generated by a stock analysis AI called "Stozy" before they are shown to users.

You MUST return the COMPLETE corrected response as valid HTML wrapped in <div class="response-content">...</div>.
Do NOT add any commentary, explanation, or text outside the HTML wrapper.
Do NOT shorten the response aggressively — preserve all accurate, relevant content.
Do NOT remove the disclaimer at the bottom.

══════════════════════════════════════════════
USER QUESTION: "${userQuery}" (regarding ${symbol})
══════════════════════════════════════════════
ORIGINAL STOZY RESPONSE:
${originalResponse}
══════════════════════════════════════════════

AUDIT CHECKLIST — apply all of the following silently before rewriting:

1. FACTUAL CONSISTENCY
   • Fix incorrect financial logic, misleading interpretations, or contradictions.
   • Ensure metrics are interpreted correctly:
     - Price BELOW EMA → resistance, NOT bullish support.
     - Debt must be judged relative to OCF and cash reserves, not in isolation.
     - Low dividend yield is NOT a risk unless the user is an income investor.
   • Remove any number not sourced from the data provided.

2. RELEVANCE
   • The FIRST section must directly answer the user's exact question.
   • Remove sections that do not address what was asked.
   • Keep only data that adds analytical value.

3. RISK QUALITY — do NOT list as major risks unless clearly justified by data:
   • Low dividend yield
   • Moderate debt without OCF context
   • Normal valuation ranges
   • Generic "competition" statements without quantified impact
   If the user asks about BIGGEST RISKS, limit to the top 3–5 most material:
   margin pressure, capex burden, regulatory changes, industry disruption, macro shocks.

4. FINANCIAL ANALYSIS DEPTH
   • Replace weak statements ("this may impact the company") with specific, data-backed insights.
   • Ensure ROCE/ROE/P/E/P/B/margins are interpreted correctly in context.
   • Avoid generic conclusions.

5. LENGTH CONTROL
   • Target 250–350 words unless a detailed breakdown was requested.
   • Remove padding, repetition, and filler phrases.

6. HTML STRUCTURE (maintain the original contract):
   • Keep <div class="response-content"> wrapper.
   • Use only allowed tags: <h2>, <h3>, <p>, <strong>, <table>, <ul><li>.
   • No markdown, no <br><br>, no inline <style>.
   • Compliance disclaimer must be the very last element inside the wrapper, verbatim.

Return the fully corrected HTML response now:`;

  const auditResponse = await Promise.race([
    model.invoke(auditPrompt),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("Auditor timeout")), 60000)
    ),
  ]);

  const audited = auditResponse?.content?.trim();
  if (!audited) return originalResponse;

  // Strip reasoning block if present
  return audited.includes("</reasoning>")
    ? audited.split("</reasoning>")[1]?.trim() || audited
    : audited;
};
...
`;
    ...
    // ── Auditor pass: QA and improve the response ────────────────────────────
    try {
      finalResponse = await runResponseAuditor(model, finalResponse, userQuery, symbol);
    } catch (auditErr) {
      console.warn("[StockAnalysisAI] Auditor pass failed, using raw response:", auditErr.message);
    }
