import os
import pyodbc  # Import the ODBC library for MSSQL
import json
import time
import requests
import re
...
app = Flask(__name__)
CORS(app)
print("stock analysis app started")
...
def connect_db():
    """Establish connection to MSSQL database using ODBC."""
    conn = pyodbc.connect(
    "Driver={ODBC Driver 18 for SQL Server};"
    "Server=nifty-sql-server.cajm4tfacz8p.ap-south-1.rds.amazonaws.com;"  # Replace with your SQL Server hostname or IP address
    "Database=stockathon;"   # Replace with your database name
    "UID=niftynodeapi;"      # Replace with your SQL Server username
    "PWD=niftysqlrds2684!!!;" # Replace with your SQL Server password
    "TrustServerCertificate=yes;"  # Added semicolon here
)
...
def search_symbol_in_tables(symbol_name):
    ...
    try:
        ...
        cursor.execute("""
                ...
                WHERE table_schema = 'dbo'  -- Default schema for MSSQL
                AND table_name LIKE 'stocks_%';
        """)
        tables = cursor.fetchall()
        print(tables)
        # Step 2: Search for the symbol_name in each table using dynamic SQL
        for table in tables:
            table_name = table[0]
...
SET @sql = N'SELECT * FROM dbo.' + QUOTENAME('{table_name}') + N' WHERE symbol_name = @symbol_name';
EXEC sp_executesql @sql, N'@symbol_name NVARCHAR(255)', @symbol_name = ?;
"""
...
def generate_analysis_with_gemini(user_input, training_data, llm):
    prompt = f"""
Your role is that you are an Stock Market Expert and a Financial Expert with 50 years of experience, and you have to give the answer to the question that is asked by the user by analyzing the below data:
...
if user asks for some analysis then use the given data to do the analysis and provide the output.
"Reply with 'sorry I am not able to answer this question' only if you do not have the correct answer to a question.".
"Always answer in clear, concise points. Each point should be direct and to the point. Every point must start with number and dot and every point must end with a comma."
"""
